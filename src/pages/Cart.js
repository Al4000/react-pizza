...
import {
	clearCartAction,
	decreasePizzaInCartAction,
	addPizzaToCartAction,
	removePizzaInCartAction
} from '../redux/actions/cart'
import emptyCart from '../assets/img/empty-cart.png'
import {Link} from 'react-router-dom'

const Cart = () => {
	const {totalPrice, totalCount, pizzas} = useSelector(({cart}) => cart)

	const pizzasGroup = Object.keys(pizzas).map(key => {
		return pizzas[key].pizzas[0]
	})
	...
	const decrementPizza = (id) => {
		dispatch(decreasePizzaInCartAction(id))
	}

	const removePizza = (id) => {
		dispatch(removePizzaInCartAction(id))
	}
						...
						{pizzasGroup.map(pizza => {
								return (
									<CartItem
										pizza={pizza}
										key={pizza.id}
										subtotal={pizzas[pizza.id].subtotal}
										count={pizzas[pizza.id].pizzas.length}
										onIncrementPizza={() => incrementPizza(pizza)}
										onDecrementPizza={() => decrementPizza(pizza.id)}
										onRemovePizza={() => removePizza(pizza.id)}
									/>
								)
							}
						)}
